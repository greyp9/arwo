...
public class ClipboardTest {
    ...
    public void testClipboardRealm() throws Exception {
        ...
        Assertions.assertEquals(0, clipboard.getFragments().size());
        final XedCursor cursorPrincipal = nav.find(principal);
        Assertions.assertNotNull(cursorPrincipal);
        Assertions.assertEquals("/ecd28/7256d/8dc37/c3dce/", cursorPrincipal.getURI());
        ...
        // copy/cut
        Assertions.assertEquals(1, ElementU.getChildren(principals).size());
        new OpClipCopy(clipboard).copy(cursorPrincipal);
        new OpClipCut(clipboard).cut(cursorPrincipal);
        logger.finest(DocumentU.toString(document));
        Assertions.assertEquals(0, ElementU.getChildren(principals).size());
        // paste
        new OpClipPaste(clipboard).paste(cursorPrincipalType);
        logger.finest(DocumentU.toString(document));
        Assertions.assertEquals(2, ElementU.getChildren(principals).size());
        new OpClipPaste(clipboard).paste(cursorPrincipalType);
        logger.finest(DocumentU.toString(document));
        final int sizeExpected = 4;
        Assertions.assertEquals(sizeExpected, ElementU.getChildren(principals).size());
        // paste empty

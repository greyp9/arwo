...
public class WebDAVHandlerPost {
    ...
    private String applySession(
        ...
        if (App.Action.FILE_CREATE.equals(action)) {
            ...
            location = PathU.toDir(httpRequest.getContextPath(), App.Cache.DAV, request.getServer());
        } else if (App.Action.TOGGLE.equals(action)) {
            PropertiesU.toggleBoolean(userState.getProperties(), Value.join(Http.Token.SLASH, App.Cache.DAV, object));

...
public class CIFSHandlerPost {
    ...
    public CIFSHandlerPost(final ServletHttpRequest httpRequest, final AppUserState userState) {
    ...
    private HttpResponse doPost() throws IOException {
        ...
        if (contentType == null) {
            ...
            final CIFSHandlerPostMultipart handler = new CIFSHandlerPostMultipart(request, userState);
    ...
    private String applySession(
        ...
        if (App.Action.FILE_CREATE.equals(action)) {
            ...
            location = PathU.toDir(httpRequest.getContextPath(), App.Servlet.WSH, request.getServer());
        } else if (App.Action.TOGGLE.equals(action)) {
            PropertiesU.toggleBoolean(userState.getProperties(), Value.join(Http.Token.SLASH, App.Cache.CIFS, object));

...
public class OpPruneTest {
    ...
    public void testPruneRealm() throws Exception {
        ...
        Assertions.assertEquals(1, xpather.getAttributes("/realm:realm/@foo-attr").size());
        Assertions.assertEquals(1, xpather.getElements("/realm:realm/realm:foo-element").size());
        // prune content
        new OpPrune().apply(new XedNav(xed).getRoot());
        logger.finest("\n" + DocumentU.toString(document));
        // content should not be there
        Assertions.assertEquals(0, xpather.getAttributes("/realm:realm/@foo-attr").size());
        Assertions.assertEquals(0, xpather.getElements("/realm:realm/realm:foo-element").size());
    }
}
